...
{
    public record class RegisterCommand:IRequest<IdentityResult>//因为API层是引用应用层，而不是反过来，所以我们不能直接使用DTO，而是用Command来传递数据，而且有些时候DTO和Command是有差异的，DTO是为了适配前端的，而Command是为了适配应用层的业务逻辑，有些参数
    {
