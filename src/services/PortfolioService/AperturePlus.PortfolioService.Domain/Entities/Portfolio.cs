...
namespace AperturePlus.PortfolioService.Domain.Entities
{
    public class Portfolio
    {
        ...
        {
            if (Galleries.Any(g => g.GalleryName.Equals(galleryName, StringComparison.OrdinalIgnoreCase)))
            {
        ...
        {
            var rmGallery = Galleries.FirstOrDefault(g => g.GalleryId == galleryId);
            if (rmGallery == null)
            {
                throw new ArgumentException("相册不存在");
            }
            Galleries.Remove(rmGallery);
        }
    }
}
